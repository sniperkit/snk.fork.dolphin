...
// Worktree returns the current worktree based on the given fs
func (r *Repository) Worktree() (*Worktree, error) {
	if r.cwt == nil {
		return nil, ErrIsBareRepository
	}

	return &Worktree{Name: r.worktree, r: r, Filesystem: r.cwt}, nil
}
...
func (r *Repository) SwitchToWorkTree(name string) (*Worktree, error) {
	if name == defaultWorkTree {
		r.Storer.SwitchToWorktree(name)
		r.worktree = name
		r.cwt = r.wt
		...
		return &Worktree{Name: name, r: r, Filesystem: r.cwt}, nil
	}
