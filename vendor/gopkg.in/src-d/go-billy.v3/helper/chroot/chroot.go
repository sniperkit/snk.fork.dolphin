...
func (fs *ChrootHelper) underlyingPath(filename string) (string, error) {
	if fs.isCrossBoundaries(filename) {
...
// there is an exception: when cross boundary should be allowed
// git worktree: gitdir a git refs to .git/worktree/gitdir
// we should allow this
func (fs *ChrootHelper) isCrossBoundaries(path string) bool {
	path = filepath.ToSlash(path)
	path = filepath.Clean(path)

	absPath := fs.Join(fs.Root(), path)
	if strings.HasSuffix(absPath, ".git") {
		_, err := fs.Stat(filepath.Join(absPath, "HEAD"))
		return err == nil
	}
