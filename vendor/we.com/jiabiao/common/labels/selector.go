...
import (
	...
	"we.com/jiabiao/common/sets"
	"we.com/jiabiao/common/validation"
...
func (r *Requirement) Matches(ls Labels) bool {
	...
	case GreaterThanOperator, LessThanOperator:
		...
		if err != nil {
			log.Infof("ParseInt failed for value %+v in label %+v, %+v", ls.Get(r.key), ls, err)
			return false
		}

		// There should be only one strValue in r.strValues, and can be converted to a integer.
		if len(r.strValues) != 1 {
			log.Infof("Invalid values count %+v of requirement %+v, for 'Gt', 'Lt' operators, exactly one value is required", len(r.strValues), r)
			return false
		}
		...
		for strValue := range r.strValues {
			...
			if err != nil {
				log.Infof("ParseInt failed for value %+v in requirement %+v, for 'Gt', 'Lt' operators, the value must be an integer", strValue, r)
				return false
			}
		}
